...
int main()
{
	...
	cout << " 3 - Подсчет суммы положительных двухзначных элементов" << endl;
...
void sum(list* p)  // Подсчет суммы положительных двузначных элементов
{
	int sm = 0;
	...
	else
	{
		while (p != NULL)
		{
			if (p->number > 0 && p->number >= 10 && p->number < 100)  // Изменили условие на положительные двузначные элементы
			{
				sm += p->number;
				found = true;
			}
