...
int countLeftChildren(Tree* t);  //Подсчет количества вершин дерева, являющихся левыми дочерними вершинами (корень дерева не учитывается)
...
int main()
{
	...
	for (;;)
	{
		...
		cout << "5 - Вывод дерева\n";
		cout << "6 - Подсчет количества вершин дерева, являющихся левыми дочерними вершинами\n";
		...
		case 6:cout << "Количество вершин дерева, являющихся левыми дочерними вершинами: " << countLeftChildren(Root) << endl;
			break;
...
int countLeftChildren(Tree* t) { //Подсчет количества вершин дерева, являющихся левыми дочерними вершинами (корень дерева не учитывается)
	...
	if (t->Left != NULL) {
		count++;
		count += countLeftChildren(t->Left);
	}

	if (t->Right != NULL) {
		count += countLeftChildren(t->Right);
	}
