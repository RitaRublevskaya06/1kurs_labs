...
void sumByLevel(Tree* p1);  //Подсчет сумм значений вершин на каждом уровне дерева
...
int main()
{
	...
	for (;;)
	{
		...
		cout << "5 - Вывод дерева\n";
		cout << "6 - Подсчет сумм значений вершин на каждом уровне дерева\n";
...
void sumByLevel(Tree* p1) {
	int sums[10] = { 0 }; // Массив для хранения сумм значений вершин на каждом уровне

	queue<pair<Tree*, int>> q; // Очередь для обхода дерева
	q.push({ p1, 0 }); // Добавляем корень дерева и его уровень (0) в очередь
	...
	while (!q.empty()) {
		...
		sums[level] += node->key; // Увеличиваем сумму на текущем уровне
	...
	// Выводим суммы значений вершин на каждом уровне
	for (int i = 0; i < 10; i++) {
		if (sums[i] != 0) {
			cout << "Сумма значений вершин на уровне " << i << ": " << sums[i] << endl;
		}
	}
}
